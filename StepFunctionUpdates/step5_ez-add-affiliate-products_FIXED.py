...
config_us_west_1 = Config(
    ...
    connect_timeout=120,
    read_timeout=180,
    retries={'max_attempts': 2, 'mode': 'adaptive'}  # Optimized
)
...
def get_affiliate_products_from_database():
    """Fetch all affiliate products from the database - FIXED FIELD NAMES"""
    try:
        ...
        for item in response['Items']:
            product = {
                ...
                'productName': item.get('productName', {}).get('S', ''),  # FIXED: Use original field name
                ...
                'inAppText': item.get('inAppText', {}).get('S', '')  # Add this field too
            }
...
def get_product_names(product_ids, available_products):
    """Get the names of products from their IDs - RESTORED ORIGINAL FUNCTION"""
    id_to_name = {p.get('id'): p.get('productName', 'Unknown') for p in available_products}  # FIXED: Use original field name
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Get product names for processing notes - FIXED FUNCTION CALL
