...
def validate_ingredient_objects(ingredient_objects, original_ingredients_count):
    ...
    return objects_count, (objects_count != original_ingredients_count)

def merge_ingredient_objects(original_json, ingredient_objects, request_id):
    """Safely merge ingredient_objects back into the original JSON with bulletproof validation."""
    
    logger.info(f"[{request_id}] Starting merge validation")
    
    # Validate that we're getting the right structure from Claude
    if not isinstance(ingredient_objects, dict):
    ...
    # Validate that original_json has the expected structure
    ...
    original_placeholder = original_json['ingredient_objects']
    logger.info(f"[{request_id}] Replacing placeholder with {len(ingredient_objects['L'])} ingredient objects")
    ...
    logger.info(f"[{request_id}] Successfully merged ingredient_objects with {final_count} items into original JSON")
    logger.info(f"[{request_id}] Merge validation: All checks passed")
    
    return original_json

def lambda_handler(event, context):
    """Main Lambda handler function with bulletproof merge validation and processing notes."""
    ...
    objects_count, count_mismatch = validate_ingredient_objects(ingredient_objects, len(ingredients_list))
    
    # Add processing notes based on results
    processing_notes = add_processing_note(processing_notes, "Step 3", f"Created {objects_count} ingredient objects from {len(ingredients_list)} ingredients")
    
    if count_mismatch:
        processing_notes = add_processing_note(processing_notes, "Step 3", f"Ingredient count changed: {len(ingredients_list)} ingredients → {objects_count} objects (Claude may have combined similar items)")
    
    # Check for potential issues
    if objects_count == 0:
        processing_notes = add_processing_note(processing_notes, "Step 3", "Warning: No ingredient objects were created")
    elif objects_count < len(ingredients_list) / 2:
        processing_notes = add_processing_note(processing_notes, "Step 3", "Warning: Significant reduction in ingredient count - review for accuracy")
    
    # Merge back into original JSON with bulletproof validation
    logger.info(f"[{request_id}] Merging ingredient objects into original JSON")
    updated_json = merge_ingredient_objects(step_output_data, ingredient_objects, request_id)
