...
# Ordered profile IDs (cross-Region pool) - CORRECTED PROFILE IDs
MODEL_PROFILES = [
...
def call_claude_for_recipe_conversion(prompt: str, request_id: str) -> Dict[str, Any]:
    ...
    # Prepare the conversation for the new converse API - CORRECTED FORMAT
    conversation = [
        {
            "role": "user",
            "content": [
                {
                    "text": prompt  # Removed "type" field - not needed in converse API
                }
            ]
        }
    ]
