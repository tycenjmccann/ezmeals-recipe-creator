...
def extract_recipe_context(step_output_data):
    ...
    # Handle both DynamoDB format and regular JSON format
    def safe_extract(data, key, default='Unknown'):
    ...
    return {
        'title': safe_extract(step_output_data, 'title', 'Unknown Recipe'),
        'cuisine_type': safe_extract(step_output_data, 'cuisineType', 'Unknown')
    }

def extract_ingredients_list(step_output_data):
    """Extract the current ingredients list from the JSON."""
    ingredients = step_output_data.get('ingredients', {})
    
    # Handle both DynamoDB format and regular JSON format
    if isinstance(ingredients, dict) and 'L' in ingredients:
        # DynamoDB format: {"L": [{"S": "ingredient1"}, {"S": "ingredient2"}]}
        ingredients_list = [item.get('S', '') for item in ingredients['L']]
    elif isinstance(ingredients, list):
        # Regular JSON format: ["ingredient1", "ingredient2"]
        ingredients_list = ingredients
    else:
        raise ValueError("No ingredients found in recipe data")
    
    # Filter out empty ingredients
    ingredients_list = [ing for ing in ingredients_list if ing and ing.strip()]
...
def extract_core_ingredient_names(ingredients_list):
    ...
    for ingredient in ingredients_list:
        ...
        cleaned = re.sub(r'\b(cups?|tablespoons?|teaspoons?|pounds?|lbs?|ounces?|oz|cloves?)\b', '', cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r'\b(chopped|diced|sliced|minced|fresh|dried|large|small|medium)\b', '', cleaned, flags=re.IGNORECASE)
...
def extract_updated_ingredients_and_changes(response_text):
    ...
    if changes_match:
        ...
        logger.info(f"Extracted changes: {changes_text}")
    
    try:
        updated_ingredients = json.loads(ingredients_str)
        ...
        return updated_ingredients, changes_text
    except json.JSONDecodeError as e:
        logger.error(f"JSON parsing failed. Extracted string: {ingredients_str}")
...
def validate_updated_ingredients(updated_ingredients, original_ingredients, original_count):
    ...
    for i, ingredient in enumerate(updated_ingredients):
        # Handle both formats: {"S": "value"} or plain string
        if isinstance(ingredient, dict):
            ...
            raise ValueError(f"Ingredient {i} must be a dictionary with 'S' field or a string, got: {type(ingredient)}")
    ...
    for i, (original, updated) in enumerate(zip(original_ingredients, updated_ingredients)):
        ...
        # Extract the actual ingredient text
        if isinstance(updated, dict) and 'S' in updated:
            updated_text = updated['S'].lower()
        elif isinstance(updated, str):
            updated_text = updated.lower()
        else:
            continue  # Skip validation for malformed ingredients
        ...
        for addition in suspicious_additions:
            if addition in updated_text and addition not in original_text:
                ...
                logger.warning(f"Updated: {updated_text}")
...
def merge_updated_ingredients(original_json, updated_ingredients, request_id):
    ...
    # Convert updated ingredients to DynamoDB format if needed
    formatted_ingredients = []
    for ingredient in updated_ingredients:
        if isinstance(ingredient, dict) and 'S' in ingredient:
            # Already in DynamoDB format
            formatted_ingredients.append(ingredient)
        elif isinstance(ingredient, str):
            # Convert string to DynamoDB format
            formatted_ingredients.append({"S": ingredient})
        else:
            logger.warning(f"[{request_id}] Unexpected ingredient format: {type(ingredient)}")
            # Try to convert to string and then DynamoDB format
            formatted_ingredients.append({"S": str(ingredient)})
    
    # Store original for comparison
    original_ingredients = original_json['ingredients']['L']
    logger.info(f"[{request_id}] Replacing {len(original_ingredients)} ingredients with {len(formatted_ingredients)} updated ingredients")
    
    # Perform the merge
    original_json['ingredients']['L'] = formatted_ingredients
    
    # Validate the merge worked correctly
    if original_json['ingredients']['L'] != formatted_ingredients:
    ...
    expected_count = len(formatted_ingredients)
...
def lambda_handler(event, context):
    """Main Lambda handler with focused ingredient name and unit standardization and detailed change tracking."""
    ...
    if not standardized_ingredients:
        ...
        processing_notes = add_processing_note(processing_notes, "Step 2", "No relevant standardized ingredients found")
        return {
            'statusCode': 200,
    ...
    # Extract and validate updated ingredients and changes
    logger.info(f"[{request_id}] Extracting updated ingredients and changes from response")
    updated_ingredients, changes_text = extract_updated_ingredients_and_changes(response_text)
    
    logger.info(f"[{request_id}] Validating updated ingredients for unwanted changes")
    suspicious_changes = validate_updated_ingredients(updated_ingredients, ingredients_list, len(ingredients_list))
    
    # Add processing notes based on results with specific changes
    if changes_text and "No changes made" not in changes_text:
        # Clean up the changes text for processing notes
        changes_lines = [line.strip() for line in changes_text.split('\n') if line.strip() and line.strip().startswith('-')]
        if changes_lines:
            processing_notes = add_processing_note(processing_notes, "Step 2", f"Standardized ingredients: {'; '.join(changes_lines)}")
        else:
            # Count changes by comparing original vs updated
            changes_count = 0
            for i, (orig, upd) in enumerate(zip(ingredients_list, updated_ingredients)):
                upd_text = upd['S'] if isinstance(upd, dict) and 'S' in upd else str(upd)
                if orig != upd_text:
                    changes_count += 1
            processing_notes = add_processing_note(processing_notes, "Step 2", f"Standardized {changes_count} ingredients")
    else:
        processing_notes = add_processing_note(processing_notes, "Step 2", "No ingredients required standardization")
    
    if suspicious_changes > 0:
        processing_notes = add_processing_note(processing_notes, "Step 2", f"Warning: {suspicious_changes} ingredients may have unwanted preparation changes")
