...
def lambda_handler(event, context):
    """Main Lambda handler with focused ingredient name and unit standardization and processing notes."""
    ...
    for original, updated in zip(ingredients_list, updated_ingredients):
        if original != updated['S']:
    ...
    if unchanged_count > 0:
        processing_notes = add_processing_note(processing_notes, "Step 2", f"Left {unchanged_count} ingredients unchanged (no matches found)")
