...
# Schema validation constants - Updated with new fields including processingNotes
REQUIRED_FIELDS = [
    "id",
    "title", 
    "dishType",
    ...
    "instaPot",
    "flagged",
    "processingNotes"  # New field for tracking processing issues
]
...
def add_processing_note(response_json: Dict[str, Any], note: str) -> None:
    ...
    logger.info(f"Added processing note: {note}")

def validate_and_fix_response_schema(response_json: Dict[str, Any]) -> bool:
    """
    ...
    try:
        ...
        optional_list_fields = [
            "ingredient_objects", "notes", "recommendedSides", "includedSides", "products", "processingNotes"
        ]
        ...
        # Add all processing notes to the JSON
        if processing_notes:
            for note in processing_notes:
                add_processing_note(response_json, note)
        else:
            # Ensure processingNotes field exists even if empty
            response_json['processingNotes'] = {"L": []}

        return True

    except KeyError as e:
        add_processing_note(response_json, f"Missing required field: {str(e)}")
        return True  # Continue processing even with missing fields
    except Exception as e:
        add_processing_note(response_json, f"Validation error: {str(e)}")
        return True  # Continue processing even with validation errors
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Construct the enhanced prompt with minimal changes to existing working prompt
        prompt = f"""
        ...
        processingNotes ("L"): Empty list [] (will be populated by processing steps)
        ...
        # Validate the response and auto-fix issues
        validate_and_fix_response_schema(response_json)

        # Log final processing notes
        if response_json.get('processingNotes', {}).get('L'):
            notes = [note['S'] for note in response_json['processingNotes']['L']]
            logger.info(f"Processing completed with notes: {notes}")
        else:
